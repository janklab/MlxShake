classdef Log4jConfigurator
    ...
    % Examples:
    %
    % exportmlx.logger.Log4jConfigurator.configureBasicConsoleLogging
    %
    % exportmlx.logger.Log4jConfigurator.setLevels({'root','DEBUG'});
    %
    % exportmlx.logger.Log4jConfigurator.setLevels({
    %     'root'    'INFO'
    %     'net.apjanke.logger.swing'  'DEBUG'
    %     });
    %
    % exportmlx.logger.Log4jConfigurator.prettyPrintLogConfiguration
    %
    % % Display fully-qualified class/category names in the log output:
    % exportmlx.logger.Log4jConfigurator.setRootAppenderPattern(...
    %    ['%d{HH:mm:ss.SSS} %p %c - %m' sprintf('\n')]);
    %
    % % Bring up the configuration GUI
    % exportmlx.logger.Log4jConfigurator.showGui
    ...
    methods (Static)
        ...
        function setLevels(levels)
            ...
            %
            % exportmlx.logger.Log4jConfigurator.setLevels(levels)
            %
            ...
            for i = 1:size(levels, 1)
                ...
                level = exportmlx.logger.Log4jConfigurator.getLog4jLevel(levelName);
